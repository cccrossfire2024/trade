# -*- coding: utf-8 -*-
"""build_gate_risk_features_v1plus4.py

在 v1 的基础上 **只新增 4 个因子**（保持 Anti-Drift / 因果）：
  1) rvol_log_96 = log1p(rvol_96)
  2) rvol_z_192  = rolling zscore(rvol_log_96, 192)
  3) nr_48       = sum(|lr_1|,48) / (|lr_48|+eps)
  4) brs_48      = (close - prior_max(high,48).shift(1)) / (atr14+eps)

其余逻辑与 v1 保持一致。

输出（out_dir 下，默认 suffix=v1p4）：
- gate_samples_{suffix}.parquet
- risk_bars_features_{suffix}.parquet
- summary_features_{suffix}.json

示例：
  python build_gate_risk_features_v1plus4.py --out_dir datasets_v1p4 --suffix v1p4
...
def rolling_zscore(s: pd.Series, win: int, minp: int) -> pd.Series:
...
def compute_gmma_struct(close: pd.Series) -> pd.DataFrame:
    ...
    out["gmma_gap"] = (med_short - med_long) / (med_long.abs() + EPS)
    out["gmma_widthS"] = (max_short - min_short) / (med_short.abs() + EPS)
    out["gmma_widthL"] = (max_long - min_long) / (med_long.abs() + EPS)

    # slope (1 bar diff)
    out["gmma_slopeS"] = (med_short.diff(1)) / (med_long.abs() + EPS)
    out["gmma_slopeL"] = (med_long.diff(1)) / (med_long.abs() + EPS)
...
def add_bar_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # returns
    for k in [1, 4, 16, 48, 64]:
        x[f"lr_{k}"] = logret(c, k)

    # Rogers-Satchell (dimensionless)
    o, h, l = x["open"].astype(float), x["high"].astype(float), x["low"].astype(float)
    ...
    x["rs_vol_rel"] = x["rs_vol"] / (x["rs_vol"].rolling(96, min_periods=96).median() + EPS)

    # volume features
    x["rvol_96"] = rvol(v, 96)

    # NEW: rvol_log_96 / rvol_z_192
    x["rvol_log_96"] = np.log1p(x["rvol_96"].clip(lower=0))
    x["rvol_z_192"] = rolling_zscore(x["rvol_log_96"], 192, minp=max(48, 192 // 3))

    # keep v1's dlogv_z (optional but we keep it to avoid changing baseline)
    x["dlogv_1"] = np.log(v + EPS).diff(1)
    x["dlogv_z"] = (x["dlogv_1"] - x["dlogv_1"].rolling(96, min_periods=96).mean()) / (
        x["dlogv_1"].rolling(96, min_periods=96).std() + EPS
    )
    ...
    x["range_atr"] = (x["high"].astype(float) - x["low"].astype(float)) / (x["atr14"] + EPS)
    x["body_atr"] = (x["close"].astype(float) - x["open"].astype(float)) / (x["atr14"] + EPS)

    # Efficiency Ratio over 16
    disp = (c - c.shift(16)).abs()
    path = (c.diff(1).abs()).rolling(16, min_periods=16).sum()
    x["er_16"] = disp / (path + EPS)
    ...
    x["vr_16_64"] = var16 / (16.0 * var1 + EPS)
    ...
    # NEW: nr_48 (noise ratio)
    k = 48
    minp = max(16, k // 3)
    x["nr_48"] = x["lr_1"].abs().rolling(k, min_periods=minp).sum() / (x["lr_48"].abs() + EPS)

    # NEW: brs_48 (breakout strength, causal)
    kk = 48
    prior_max = x["high"].astype(float).rolling(kk, min_periods=max(16, kk // 3)).max().shift(1)
    x["brs_48"] = (x["close"].astype(float) - prior_max) / (x["atr14"] + EPS)
...
def summarize_window(df_feat: pd.DataFrame, t_center: pd.Timestamp, w_pre: int, w_post: int) -> dict:
    ...
    cols = [
        ...
        "vov_96",
        # NEW 4
        "rvol_log_96", "rvol_z_192", "nr_48", "brs_48",
    ]
...
def main():
    ...
    ap.add_argument("--out_dir", type=str, default="datasets_v1p4")
    ap.add_argument("--suffix", type=str, default="v1p4")
    ...
    for sym in sorted(uni["symbol"].unique()):
        ...
        print(f"[FEAT v1+4] computing bar features for {sym} ...")
    ...
    # ---- Gate samples ----
    print("[GATE v1+4] building event snapshots ...")
    gate_rows = []
    ...
    gate_path = out_dir / f"gate_samples_{suffix}.parquet"
    gate_df.to_parquet(gate_path, index=False)

    # ---- Risk bar features ----
    print("[RISK v1+4] building risk bars features ...")
    ...
    risk_feat_path = out_dir / f"risk_bars_features_{suffix}.parquet"
    ...
    summary = {
        ...
        "added_features": ["rvol_log_96", "rvol_z_192", "nr_48", "brs_48"],
    }

    summary_path = out_dir / f"summary_features_{suffix}.json"
